...
pub(super) fn attach(report: &mut AnalysisReport) {
    let members: Vec<_> = report.files.iter().map(|file| filefacts::package_context::SourceMember {
        path: &file.path, values: &file.kv,
    }).collect();
...
pub(super) fn attach_go(report: &mut AnalysisReport, sources: &[(String, String)], incomplete: bool) {
