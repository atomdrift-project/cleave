//! `class.*` kv subtree — Java `.class` structural metadata
//! (version, access flags, class hierarchy, SourceFile attribute).
//! Cheap header-only walk; complements the capability/string
//! analysis in `analyzers::java_class`. Schema is the [`ClassKv`]
//! struct.
...
/// Sparse view of the constant pool. We only retain the entries
/// the kv extractor actually reads (`Utf8` strings and `Class`
/// pointers); skipping `Other` placeholders avoids allocating one
/// `Vec` slot per irrelevant entry — meaningful on fat JARs whose
/// constant pools run into the tens of thousands.
#[derive(Default)]
struct ConstantPool {
    utf8: FxHashMap<u16, String>,
    class: FxHashMap<u16, u16>,
}
...
/// Strongly-typed `class.*` kv tree. Optional fields use
/// `skip_serializing_if` so absent attributes don't bloat the output.
...
pub(crate) fn extract(data: &[u8]) -> Option<Value> {
    ...
    let mut interface_idx: Vec<u16> = Vec::with_capacity(interfaces_count);
    for _ in 0..interfaces_count {
        interface_idx.push(read_u16(data, &mut pos)?);
    }
    ...
    let kv = ClassKv {
        ...
        minor_version,
        access_flags,
        constant_pool_size: cp_count as u32,
        this_class: cp.class_name(this_idx).map(str::to_string),
        super_class: cp.class_name(super_idx).map(str::to_string),
        interfaces: interface_idx
            .iter()
            .filter_map(|i| cp.class_name(*i).map(str::to_string))
            .collect(),
...
fn parse_attributes(data: &[u8], pos: &mut usize, cp: &ConstantPool) -> ClassAttributes {
...
fn parse_constant_pool(data: &[u8], pos: &mut usize, count: usize) -> Option<ConstantPool> {
