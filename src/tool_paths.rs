...
///
/// PATH is checked before platform fallback locations, and the resulting path
/// or miss is cached. Callers pass the returned path to `Command::new` so the
/// child process does not repeat executable discovery.
#[must_use]
