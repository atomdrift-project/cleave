...
pub struct JpegMetrics {
    /// Bytes appended after the JPEG EOI marker
    ///
    ...
    /// Total bytes in APP1 EXIF metadata markers
    ///
    ...
    /// Number of APP0–APP15 metadata segments
    ///
    /// High counts hint at unusual metadata layering (e.g. Photoshop multi-section blobs).
    #[serde(default, skip_serializing_if = "is_zero_u32")]
    pub app_segment_count: u32,
    /// Number of JFIF COM comment markers
    ///
    /// Multiple COMs in one JPEG is unusual and a classic stego carrier.
    #[serde(default, skip_serializing_if = "is_zero_u32")]
    pub com_count: u32,
    /// Number of DQT quantization table markers
    ///
    /// Useful baseline for tools like jsteg that mutate them.
    #[serde(default, skip_serializing_if = "is_zero_u32")]
    pub dqt_count: u32,
    /// Number of DHT (Huffman table) markers.
    #[serde(default, skip_serializing_if = "is_zero_u32")]
    pub dht_count: u32,
    /// Number of SOI start-of-image markers seen
    ///
    /// Legitimate JPEGs have exactly 1; >1 means concatenated images.
    #[serde(default, skip_serializing_if = "is_zero_u32")]
    pub soi_count: u32,
    /// Bytes in EXIF MakerNote vendor-opaque tags
    ///
    /// Vendor-opaque blobs ranging up to a few KB on real cameras; values much larger
