...
gemini-2.5-pro, gemini-2.5-flash. Popular choices:
  ...
  opencode: gpt-4.1, gpt-4.1-mini, o4-mini, o3, moonshotai/kimi-k2.5`)
	...
	timeout := flag.Duration("timeout", 28*time.Minute, "Maximum time for each AI invocation")
	...
	if *useCargo {
		...
		// Skip jemalloc on illumos/solaris where tikv-jemalloc-sys doesn't build.
		args := []string{"build", "--release"}
		if runtime.GOOS != "illumos" && runtime.GOOS != "solaris" {
			args = append(args, "--features", "jemalloc")
			fmt.Fprint(os.Stderr, "Building release binary with cargo build --release --features jemalloc...\n")
		} else {
			fmt.Fprint(os.Stderr, "Building release binary with cargo build --release (no jemalloc on this OS)...\n")
		}
